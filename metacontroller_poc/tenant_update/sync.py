...
def old_workflow(job):
  wf = copy.deepcopy(job)
  wf['apiVersion'] = 'argoproj.io/v1alpha1'
  wf['kind'] = 'Workflow'
  wf['metadata'] = {}
  wf['metadata']['name'] = '%s-dj' % (job['metadata']['generateName'])
  wf['metadata']['namespace'] = 'argo'
  wf['spec'] = {}
  wf['spec']['entrypoint'] = copy.deepcopy(job['spec']['entrypoint'])
  wf['spec']['serviceAccountName'] = copy.deepcopy(job['spec']['serviceAccountName'])
  wf['spec']['templates'] = copy.deepcopy(job['spec']['templates'])
...
HTTPServer(('', 8005), Controller).serve_forever()
